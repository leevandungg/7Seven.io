...
public class HomeController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse response)
        ...
        // Product Đĩa game
        List<Product> product_diagame = productService.getProductById(3);
        req.setAttribute("product_cds", product_diagame);

        // Product máy chơi game
        List<Product> product_maychoigame = productService.getProductById(1);
        req.setAttribute("product_gameconsole", product_maychoigame);
        // Check sản phẩm khác
        List<Product> product_others = productService.getProductById(4);
        req.setAttribute("product_others", product_others);

        // Check sản phẩm gaming
        List<Product> product_gaming = productService.getProductById(2);
