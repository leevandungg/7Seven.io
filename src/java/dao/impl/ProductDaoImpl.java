...
public class ProductDaoImpl extends connectDB implements ProductDao {
    ...
    public int getTotalProduct() {
        String sql = "SELECT COUNT(id) FROM product where catalog_id != 5 and catalog_id != 6 and catalog_id != 7";
        Connection conn = connectDB.getConnect();
        try {
            PreparedStatement ps = conn.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
    ...
    public List<Product> pagingProducts(int index) {
        List<Product> products = new ArrayList<Product>();
        String sql = "SELECT * FROM product where catalog_id != 5"
                + " and catalog_id != 6"
                + " and catalog_id != 7 ORDER BY id OFFSET ? ROWS FETCH NEXT 12 ROWS ONLY";
        Connection conn = connectDB.getConnect();
        try {
            PreparedStatement ps = conn.prepareStatement(sql);
